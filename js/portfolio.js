...
// This function idplays the first image in the slideshow when the page loads
var slideIndex = 1;
...
function showSlides(n) {
    ...
    if (n > slides.length) {slideIndex = 1} // If n (the number passed into the function) is greater than the length of the array "alides", the slideIndex is set to 1
...
document.addEventListener("click", function(event) {
    // Here we state that if the click happens on the cancel button OR anywhhere that is not the contact form AND the click does not happen on any element with the contact class then call the closeForm() function
